...
const SelectedParkPanel = ({ park, onClose, showCloseButton = true, closeLabel = 'Clear' }) => {
  ...
  return (
    <section
      ref={panelRef}
      id="selected-park"
      className="scroll-mt-20 rounded-2xl border border-slate-700/90 bg-slate-900/80 p-4 shadow-xl shadow-black/30 sm:p-8"
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3 sm:mb-6">
        <div className="min-w-0 flex-1">
          ...
          <h2
            ...
            className="break-words text-xl font-bold tracking-tight text-slate-100 sm:text-3xl"
          >
        ...
        {showCloseButton && (
          <button
            type="button"
            onClick={onClose}
            className="inline-flex min-h-10 shrink-0 items-center rounded-xl border border-slate-600 px-3 py-2 text-sm text-slate-300 transition-colors hover:border-slate-500 hover:text-white"
          >
            {closeLabel}
          </button>
        )}
        ...
        <div className="order-2 space-y-6 lg:order-1">
              ...
              <ul className="flex flex-wrap gap-2">
                {features.map((f) => (
                  <li
                    ...
                    className="rounded-full border border-slate-700 bg-slate-800/80 px-3 py-1.5 text-sm text-slate-200"
                  >
          ...
          <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:flex-wrap">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-amber-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-amber-500 sm:w-auto"
            >
              Get directions
            </a>
          </div>
